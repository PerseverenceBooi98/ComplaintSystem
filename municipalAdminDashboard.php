...
// Check if the user is logged in and is a municipal admin
if (!isset($_SESSION['admin_id']) || $_SESSION['admin_type'] !== 'municipal') {
    header("Location: admin.Login.php");
    exit();
}

// Fetch municipality ID from session
$municipality_id = $_SESSION['municipality_id']; // Use municipality_id from session

// Fetch municipality name
$municipality_query = $conn->prepare("SELECT municipality_name FROM municipalities WHERE municipality_id = ?");
$municipality_query->bind_param("i", $municipality_id);
$municipality_query->execute();
$municipality_result = $municipality_query->get_result();
$municipality = $municipality_result->fetch_assoc();

// Debugging: Check if municipality was found
if (!$municipality) {
    die("Municipality not found for ID: " . htmlspecialchars($municipality_id));
}

// Fetch complaints for the municipality (only unresolved)
$complaints_query = $conn->prepare("SELECT c.* FROM complaints c WHERE c.municipality_id = ? AND c.status != 'Resolved'");
$complaints_query->bind_param("i", $municipality_id);
$complaints_query->execute();
$complaints_result = $complaints_query->get_result();

// Fetch statistics
$stats_query = $conn->prepare("SELECT COUNT(*) as total, SUM(status = 'Resolved') as resolved FROM complaints WHERE municipality_id = ?");
$stats_query->bind_param("i", $municipality_id);
$stats_query->execute();
...
<body>
    ...
    <div class="content">
        <h1>Hello Admin of <?php echo htmlspecialchars($municipality['municipality_name']); ?></h1>
        <h2>Complaints Submitted to Your Municipality</h2>
